...
## [2026-01-30] - 실시간 기업 재무 분석 고도화 및 분석 리포트 기능 구현

### 1. 챗봇 및 뉴스 분석 기능 고도화
*   **Context Window 최적화**: LLM에 전달되는 대화 내역을 최근 3쌍(총 6개 메시지)으로 제한하여 토큰 비용 절감 및 최신 맥락 유지.
*   **조건부 UI 렌더링**: '기사 분석 모드' 토글을 뉴스 페이지(`/news`)에서만 활성화되도록 `useLocation` 기반 로직 적용.
*   **전략 보고서 생성 및 내보내기**:
    *   20년 경력의 전략 기획 베테랑 페르소나를 시스템 프롬프트에 주입하여 고품질 분석 수행.
    *   생성된 리포트를 Markdown(.md) 파일로 즉시 다운로드할 수 있는 기능 구현.

### 2. 재무제표 분석(FS Analysis) 시스템 구축 (Level 2)
*   **실시간 데이터 연동**: 로컬 JSON 파일 기반에서 OpenDART API(`fnlttSinglAcnt`) 실시간 호출 구조로 전환.
*   **DB 기반 기업 검색 엔진**:
    *   PostgreSQL `dart_corps` 테이블과 연동된 실시간 자동 완성 검색바 구현.
    *   상장사(`stock_code` 보유) 필터링 및 스마트 정렬(일치도 > 시작문자 > 가나다순) 적용.
*   **시각화 구조 개선**:
    *   차트 축 구성을 [X축: 계정 / 범례: 연도]로 변경하여 항목별 3개년 추이 비교 직관성 강화.
    *   데이터 단위를 '백만원'으로 스케일링하고 Y축 레이아웃 및 제목 표기 최적화.
*   **AI 재무 진단 리포트**:
    *   재무/전략 전문가 페르소나 기반의 통합 재무 진단 기능 추가.
    *   Markdown 테이블 렌더링(`remark-gfm`) 지원 및 전문 리포트(Paper) 스타일의 전용 UI 적용.

### 3. UI/UX 및 안정성 개선
*   **반응형 레이아웃**: 대시보드 높이를 뷰포트 기반(`calc(100vh - 140px)`)으로 설정하여 노트북 등 낮은 해상도에서 입력창 잘림 현상 해결.
*   **컴포넌트 리팩토링**: 인라인 스타일 구문 오류 수정 및 뉴스 그리드 레이아웃 적용.